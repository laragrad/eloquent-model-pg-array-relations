...
namespace Laragrad\Relations;

use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Collection;
use Laragrad\Support\PgHelper;

class BelongsToManyArrays extends PgArrayRelation
{
    ...
    public function attach($ids, $touch = true)
    {
        if (!is_null($ids)) {
            ...
            $models = $this->related->getModel()
                ->whereIn($this->query->qualifyColumn($this->query->getModel()->getKeyName()), $ids)
                ->get();
            ...
            foreach ($models as $model) {
                ...
                $model
                    ->setAttribute(
                        $this->arrayField,
                        array_values(array_unique(array_merge($val, [$parentId])))
                    )
                    ->save();
            }
        }
        ...
        if (!is_null($ids)) {
            ...
            foreach ($models as $model) {
                ...
                $model
                    ->setAttribute(
                        $this->arrayField,
                        array_values(array_unique(array_diff($val, [$parentId])))
                    )
                    ->save();
            }
        }
